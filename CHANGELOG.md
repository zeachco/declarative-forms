...
- e6e15da: removed invalidChildren from SchemaNode (use getInvalidChildren instead) removed isValid argument from updateParent and onChildChange

### Patch Changes

- 4c2d9a7: removed husky from dependencies
- 9e908de: Fixes getInvalidChildren count when mixed with cloned nodes and called on a root node
- 732a4da: removed eslint/polyfills for chrome99..101, edge100..101, firefox98..100, opera84..86 and added new browsers

## 3.1.1

### Patch Changes

- 342aa2f: documentation is now autogenerated on release and saved into the current branch rather than the doc-site one, allowing for a snapshot of the doc for each release

## 3.1.0

### Minor Changes

- fb6f21a: Updated react's peer and dev dependencies to fit web

## 3.0.2

### Patch Changes

- f62c84a: Disabled jekyll for docsite

## 3.0.1

### Patch Changes

- 3853a02: Fixed a bug where the installation would fail because of husky when running `yarn add @zeachco/declarative-forms`. Husky is a tool that's meant to be use for the maintenance of the repo itself and not used by the consumer's repository

## 3.0.0

### Major Changes

- 24c3d3c: removed validity cache, deprecated SchemaNode.invalidChildren. Moving the structure to context state to reduce the amount of mismatching information across instances of SchemmaNode

### Patch Changes

- ff401b9: Adds git pre-commit hook for pr and release content quality
- b303344: Library no longer yield errors when instanciating cloned nodes with polymorphic values

## 2.3.0

### Minor Changes

- 9bfd4e5: Fixed onChange when called on polymorphic nodes to also update their children nodes
